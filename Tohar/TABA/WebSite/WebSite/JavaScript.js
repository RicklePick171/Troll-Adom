function checkForm() {
    ...
    var dotSign = MEmail.indexOf('.', MEmail);
    var dotSignlast = MEmail.lastIndexOf('.', MEmail);
    ...
    if (MEmail.length < 6) {
        ...
        document.getElementById("mEmail").value = "  מיקומו של ה-@ במקום הלא נכון או לא קיים.";
        document.getElementById("mEmail").style.display = "inline";
        return false;
    }
    else if (dotSign - atSign < 2) {
        document.getElementById("mEmail").value = "הנקודה במקום לא הגיוני או לא קיימת.";
        document.getElementById("mEmail").style.display = "inline";
        return false;
    }
    ...
    else if (MEmail.charAt(MEmail.length-1) == '.') {
...
function isVaildString(str) {
    ...
    var badStr = "$%^&*()_+[]{}<>?אבגדהוז,חטיכךלמםנןסעפצקרשת";
    var i = 0, p, ch;
